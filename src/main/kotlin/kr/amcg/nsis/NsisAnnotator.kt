...
class NsisAnnotator : Annotator {
    ...
    override fun annotate(element: PsiElement, holder: AnnotationHolder) {
        ...
        // 3) 없는 파일을 가리키는 경로 인자
        val baseDir = element.virtualFile?.parent
        if (baseDir != null) {
            for (ref in analysis.pathRefs) {
                val r = range(ref.startOffset, ref.endOffset) ?: continue
                if (NsisPaths.resolve(ref, baseDir, analysis.pathRefs) != null) continue
                val normalized = NsisPaths.normalize(ref.path)
                if (ref.directive.equals("!include", ignoreCase = true)) {
                    // !include 는 스크립트 폴더가 아니라 검색 경로 전체에서 찾는다
                    if (NsisPaths.shouldWarnMissingInclude(ref.path)) {
                        holder.newAnnotation(
                            HighlightSeverity.WARNING,
                ...
                val shown = if (File(normalized).isAbsolute) normalized else "${baseDir.path}/$normalized"
                holder.newAnnotation(
                    HighlightSeverity.WARNING,
                    "${ref.directive} 대상이 없습니다: $shown",
                ).range(r).create()
            }
        }
